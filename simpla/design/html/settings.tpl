...
<input type=hidden name="session_id" value="{$smarty.session.id}">
		...
		<div class="block">
			...
			<ul>
                ...
                <li><label class=property>Директория размещения сайта</label><input name="sub" class="simpla_inp" type="text" value="{$settings->sub|escape}" /></li>
		...
		<div class="block layer">
			...
			<ul>
				<li>
					<label for="dpd_coefficient" class=property>Коэффициент расчета стоимости доставки DPD (%): </label>
					<input name="dpd_coefficient" class="simpla_inp" type="text" size="25" value="{$settings->dpd_coefficient|escape}" />
				</li>
			</ul>
		</div>
		<!-- Параметры (The End)-->
		...
		<!-- Параметры -->
		<div class="block layer">
			<h2 title="Единовременно может быть включена только одна глобальная акция">Включение/выключение глобальных акций, распродаж и т.п.</h2>
			<p>Единовременно может быть включена только одна глобальная акция</p><br>
			<ul>
				<li>
					<label for="action_time" class=property>Выставите код (например: 0 - все выключенно, 1 - распродажа)</label>
					<input placeholder="Напиши сюда код статуса. 0 - Выключенно. 1 - Распродажа" name="action_time" class="simpla_inp" type="text" size="25" value="{$settings->action_time}" />
				</li>
			</ul>
		</div>
		<!-- Параметры (The End)-->
