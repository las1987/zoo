		...
		<tr>
			<td>
				<span class=view_purchase>
					...
					{foreach $my_purchases_array as $my_purchases}
						{assign var="a" value=$my_purchases.variant_id}
						{assign var="b" value=$purchase->variant_id}
					...
					{$purchase->product->brand} {$purchase->product_name} {$purchase->variant_name} {if $purchase->sku} (артикул {$purchase->sku}){/if}
