...
class Notify extends Simpla
{
	...
	public function email_order_user($order_id)
	{
			...
			// Пункт самовывоза
			$pickuppoint = $this->PickupPoints->get_pickuppoint($order->pickuppoint_id);
	...
	public function email_order_admin($order_id)
	{
			...
			$subject = $this->design->get_var('subject');
			$this->email($this->settings->order_email, "Заказ №".$order_id, $email_template, $this->settings->notify_from_email);
    ...
    public function email_call_order_admin($call_order_id)
    {
        if(!($call_order = $this->call_orders->get_call_order(intval($call_order_id))))
            return false;

        $this->design->assign('call_order', $call_order);

        // Отправляем письмо
        $email_template = $this->design->fetch($this->config->root_dir.'simpla/design/html/email_call_order_admin.tpl');
        $subject = $this->design->get_var('subject');
        $this->email($this->settings->comment_email, $subject, $email_template, $this->settings->notify_from_email);
    }
